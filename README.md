...
System Update: Ensure all packages are current using sudo apt update && sudo apt upgrade -y.
...
Build Image Job: Triggered by the Clone Job; it enters the workspace and runs docker build -t app-image ..

Run Container Job: Triggered by the Build Job; it stops/removes old containers and starts a new one: docker run -d -p 80:3000 --name running-app app-image.
...
Verification: Use docker ps to ensure the container status is "Up" and port mappings are correct.
